const {
  ...
  findSafePlaces,
  adviceForAlex
} = require("../src/savePlaces");

// Please enable Level 1, 2, 3-Tests by replacing describe with describe!
...
describe("Spy Places Level 1 - convert coordinates", () => {
  ...
  it("one agent casts to array", () => {
    ...
    expect(convertCoordinates(agents)).toEqual(
  ...
  it("some agents casts to array", () => {
    ...
    expect(convertCoordinates(agents).length).toEqual(agents.length);
    expect(convertCoordinates(agents)).toEqual(
      expect.arrayContaining([[1, 5], [2, 1], [9, 6]])
    );
  });
...
describe("Spy Places Level 2 - find save places", () => {
  it("some places are save if agents are some", () => {
    ...
    expect(findSafePlaces(agents)).toEqual(
      expect.arrayContaining([[0, 9], [0, 7], [5, 0]])
    );
  });
  it("some places are save if agents are some", () => {
    ...
    expect(findSafePlaces(agents).length).toEqual(3);
    expect(findSafePlaces(agents)).toEqual(
      expect.arrayContaining([[5, 7], [6, 6], [7, 5]])
    );
  });
...
describe("Spy Places Level 3 - find edge cases and give advice to Alex", () => {
  it("should be replaced with a descriptive message", () => {
    const agents = [];
    expect(adviceForAlex(agents)).toEqual(
  ...
  it("some places are save if agents are some", () => {
    ...
    expect(adviceForAlex(agents)).toEqual(
      expect.arrayContaining(["A10", "A8", "F1"])
    );
  });
  it("some places are save if agents are some", () => {
  ...
  it("some places are save if agents are some", () => {
    ...
    expect(adviceForAlex(agents).length).toEqual(3);
    expect(adviceForAlex(agents)).toEqual(
      expect.arrayContaining(["F8", "G7", "H6"])
    );
  });
  it("one save place", () => {
    const agents = ["A1"];
